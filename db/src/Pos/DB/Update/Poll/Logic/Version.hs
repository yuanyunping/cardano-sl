...
verifyBlockAndSoftwareVersions upId up = do
    ...
    case (bvc, svc) of
        -- We give preference to BlockVersion errors, not for any
        -- particular reason, but because one must come first.
        -- This covers 3 of the 3x3=9 cases.
        (BVInvalid, _) -> do
            lastAdopted <- getAdoptedBV
            throwError
                $ PollBadBlockVersion upId (upBlockVersion up) lastAdopted
        -- This covers 2 of the 3x3=9 cases.
        (_, SVInvalid) -> do
            throwError
                $ PollWrongSoftwareVersion mAdoptedSV app
                                           (svNumber proposedSV) upId
        -- These cover the remaining 4 of the 3x3=9 cases.
